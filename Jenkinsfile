pipeline {
    ...
    stages {
        ...
        stage('Docker Build') {
            steps {
                sh "docker build . -t reshma2307/hiring-app:$BUILD_NUMBER"
            }
        }
        stage('Docker Push') {
            steps {
                withCredentials([string(credentialsId: 'docker-hub', variable: 'hubPwd')]) {
                    sh "docker login -u reshma2307 -p ${hubPwd}"
                    sh "docker push reshma2307/hiring-app:$BUILD_NUMBER"
                }
            }
        }
        stage('Checkout K8S manifest SCM'){
            steps {
              git branch: 'main', url: 'https://github.com/reshma2307/Hiring-app-argocd.git'
            }
        } 
